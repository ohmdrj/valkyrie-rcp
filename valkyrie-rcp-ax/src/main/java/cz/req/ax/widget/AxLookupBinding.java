...
public abstract class AxLookupBinding extends CustomBinding {
     ...
     * Always pop-up.
     */
    public static final int AUTOPOPUPDIALOG_ALWAYS = AUTOPOPUPDIALOG_UNIQUE_MATCH + AUTOPOPUPDIALOG_NO_MATCH + AUTOPOPUPDIALOG_MULTIPLE_MATCH;
    /**
     ...
     */
    private final AxAbstractEditor dataWidget;
    /**
    ...
    protected Object initializeDataEditor() {
        ...
        if ((ref != null) && textFieldValue.equals(getObjectLabel(ref))) {
            return getDataWidget().setSelectedSearch(ref);
        }
        return getDataWidget().setSelectedSearch(createFilterFromString(textFieldValue));
    }
            ...
            protected void doExecuteCommand() {
                if (AxLookupBinding.this.propertyChangeMonitor.proceedOnChange()) {
                    if (dataEditorDialog == null) {
                        dataEditorDialog = new TitledWidgetApplicationDialog(getDataWidget(),
                            ...
                            protected boolean onSelectNone() {
                                getDataWidget().getMaster().clearSelection();
                        ...
                        AxApp.applicationConfig().applicationObjectConfigurer().configure(dataEditorDialog, getSelectDialogId());
                    }
                    ...
                    if (getDialogSize() != null) {
                        ...
                        dataEditorDialog.getDialog().setPreferredSize(getDialogSize());
                    }
                    dataEditorDialog.showDialog();
                }
            }
        };
        AxApp.applicationConfig().commandConfigurer().configure(selectDialogCommand);
     ...
     */
    protected AxAbstractEditor getDataWidget() {
     ...
     */
    protected boolean onFinish() {
        setValue(getDataWidget().getMaster().getSelection(), false);
        return true;
    }

    private void setValue(Object value, boolean doLoadDetailedObject) {
        if (value != null && !loadDetailedObject) {
            value = getDataWidget().getDataProvider().getSimpleObject(value);
        } else if (value != null && doLoadDetailedObject) {
            value = getDataWidget().getDataProvider().getDetailObject(value, false);
        }
    ...
    private class ReferableDataEditorViewCommand extends ActionCommand {
        ...
        ReferableDataEditorViewCommand() {
            ...
            AxApp.applicationConfig().commandConfigurer().configure(this);
        }
